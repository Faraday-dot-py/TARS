...
ZERO_IP = "10.42.0.103"       # set to your Pi Zero W IP
...
class Temp(Node):
    ...
    def on_cmd(self, msg: Float32MultiArray):
        # Expect: [enable, hip, knee, ankle]
        data = list(msg.data)
        if len(data) < 4:
            self.get_logger().warn("cmd expects 4 floats: [enable, hip, knee, ankle]")
            return

        enable = bool(data[0] >= 0.5)
        hip = float(data[1])
        knee = float(data[2])
        ankle = float(data[3])
        ...
        pkt = {
            ...
            "seq": self.cmd_seq,
            "enable": enable,
            "setpoints": {"hip": hip, "knee": knee, "ankle": ankle},
